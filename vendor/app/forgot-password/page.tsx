      ...
      <div className="max-w-md w-full bg-white p-12 md:p-16 rounded-[3rem] shadow-2xl border border-[#eb9a05]/10 relative z-10 animate-fade-in-up">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-[#eb9a05]/10 border border-[#eb9a05]/20 text-[#eb9a05] mb-8">
            ...
            <span className="text-[10px] font-black tracking-[0.3em] uppercase">Reset Password</span>
          </div>
          <h2 className="text-4xl font-playfair font-black text-[#002147] leading-tight">Forgot Password</h2>
          <p className="text-gray-400 text-xs font-bold tracking-widest uppercase mt-4">Enter your email address to receive a password reset link.</p>
        </div>
        ...
        <form className="space-y-8" onSubmit={formik.handleSubmit}>
          ...
          >
            <span className="text-xs md:text-sm font-black tracking-[0.2em] uppercase">{formik.isSubmitting ? "Sending..." : "Send Reset Link"}</span>
            {!formik.isSubmitting && <ArrowRight className="w-4 h-4 md:w-5 md:h-5 transition-transform group-hover:translate-x-2" />}
          </button>
